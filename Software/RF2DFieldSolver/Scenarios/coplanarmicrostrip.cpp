...
CoplanarMicrostrip::CoplanarMicrostrip()
{
...
ElementList *CoplanarMicrostrip::createScenario()
{
    auto list = new ElementList();

    if(ui->autoArea->isChecked()) {
        ui->xleft->setValue(-std::max({substrate_height * 5, width, gap}));
        ui->xright->setValue(std::max({substrate_height * 5, width, gap}));
...
QPixmap CoplanarMicrostrip::getImage()
{
    return QPixmap(":/images/coplanar_microstrip.png");
}
