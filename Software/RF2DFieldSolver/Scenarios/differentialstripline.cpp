...
DifferentialStripline::DifferentialStripline()
{
...
ElementList *DifferentialStripline::createScenario()
{
    ...
    if(ui->autoArea->isChecked()) {
        ui->xleft->setValue(-std::max(thickerSubstrate * 5, (width + gap/2) * 3));
        ui->xright->setValue(std::max(thickerSubstrate * 5, (width + gap/2) * 3));
...
QPixmap DifferentialStripline::getImage()
{
    return QPixmap(":/images/stripline_differential.png");
}
